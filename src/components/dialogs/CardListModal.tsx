...
import { DECK_HERMIT, DECK_LIGHT, DECK_SHADOW } from "../../data/cards";
...
export default function CardListModal({ isOpen, onClose }: CardListModalProps) {
  if (!isOpen) return null;

  const hermitCards = DECK_HERMIT.filter(c => !c.isEquipment);
  const lightCards = DECK_LIGHT.filter(c => !c.isEquipment);
  const shadowCards = DECK_SHADOW.filter(c => !c.isEquipment);
      ...
      >
        ...
        <div className="sticky top-0 bg-neutral-900 border-b border-neutral-800 px-6 py-4 flex items-center justify-between z-10">
          <div className="flex items-center gap-2 text-rose-500">
            ...
            <h2 className="text-xl font-bold font-sans tracking-tight">Danh Sách Thẻ Bài</h2>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 space-y-8 font-sans">
          ...
          <section className="space-y-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {hermitCards.map((card) => (
                <div key={card.id} className="bg-emerald-950/20 border border-emerald-900/30 rounded-xl p-4 space-y-2">
